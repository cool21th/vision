...
def imsave(path, img):
    img = np.clip(img, 0, 255).astype(np.float)
...
with tf.Graph().as_default(), tf.Session() as sess, tf.device('/cpu:0'):
    ...
    print("Type of 'feature' is ", type(content_features))
    print("Shape of 'feature' is %s" %(content_features.shape,))
...
plt.savefig("images/Stylized.png")
plt.show()
